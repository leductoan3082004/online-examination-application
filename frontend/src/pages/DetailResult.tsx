...
const DetailResult = () => {
    ...
    useEffect(() => {
        const loadDetail = async () => {
            if (!attemptId) return;
            try {
                const result = await studentService.getAttemptDetail(attemptId);
                setData(result);
            } catch (error) {
            ...
            <div className="max-w-4xl mx-auto">
                        ...
                        <div className="flex justify-between items-start flex-wrap gap-6">
                            <div className="space-y-4">
                                ...
                                <div className="flex flex-wrap gap-4 pt-2">
                                    <div className="flex items-center gap-2 bg-[#F8F9FA] px-4 py-2 rounded-md border border-[#E9ECEF]">
                                        ...
                                        <span className="text-sm text-[#1F1F1F]">Instructor: <b className="font-semibold text-[#0056D2]">{data.teacherName || "Academy Team"}</b></span>
                                    </div>
                                    <div className="flex items-center gap-2 bg-[#F8F9FA] px-4 py-2 rounded-md border border-[#E9ECEF]">
                                        <Clock size={16} className="text-[#636363]" />
                                        <span className="text-sm text-[#636363]">{new Date(data.submittedAt).toLocaleTimeString('vi-VN')} • {new Date(data.submittedAt).toLocaleDateString('vi-VN')}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="relative w-32 h-32 flex items-center justify-center rounded-full border-8 border-[#F0F4FA] bg-white">
                                <div className="text-center">
                                    <p className="text-3xl font-black text-[#0056D2]">{Math.round(data.percentage)}%</p>
                ...
                <div className="space-y-8">
                    <h2 className="text-xl font-bold text-[#1F1F1F] flex items-center gap-2 px-2">
                        Review Questions <span className="bg-[#D1D7DC] text-[#1F1F1F] text-xs px-2 py-0.5 rounded-full">{data.questions?.length} Items</span>
                    </h2>

                    {data.questions.map((q: any, idx: number) => (
                        <div key={q.questionId} className="bg-white rounded-xl border border-[#D1D7DC] overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                                ...
                                <div className="grid gap-4">
                                    ...
                                    <div className={`p-5 rounded-lg border-2 flex items-center gap-4 ${q.isCorrect ? 'bg-white border-[#008148]' : 'bg-white border-[#B32D0F]'}`}>
                                        ...
                                        <div>
                                            ...
                                            <p className="text-lg font-medium">{q.selectedOptionText}</p>
                                        </div>
                                    </div>
                                        ...
                                        <div className="mt-4 p-6 bg-[#F0F4FA] rounded-lg border border-[#CFE1F6]">
                                            <div className="flex gap-3 items-start">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-[#0056D2] font-bold text-lg mb-2">{q.correctOptionText}</p>
                ...
                <div className="mt-16 text-center border-t border-[#D1D7DC] pt-12">
                    <p className="text-sm text-[#636363] mb-8 font-medium italic">
                        "Your progress is a marathon, not a sprint. Great effort, Huy!"
                    </p>
                    <button
                        onClick={() => navigate('/my-results')}
